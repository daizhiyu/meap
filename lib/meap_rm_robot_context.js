...
var CMIOREDIS = require("./meap_ioredis");
...
RBTContext.prototype.getCacheCC = function (url, cb, st) {
    var self = this;
    self.Context.CacheManIORedis.getCacheControl(url, st, function (err, obj) {
            ...
            if (obj) {
                ...
                if (CC.EXP) {
                    if ((new Date()) < (new Date(CC.EXP))) {
                        self.Context.CacheManIORedis.getCache(url, st, function (err, obj) {
                            if (err) {
                                cb("CHECK", CC);
                            } else {
                                cb("CACHE", obj);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
//
//              Manage the Public cache  by daizhiyu
//
//////////////////////////////////////////////////////////////////////////////////////////////////
RBTContext.prototype.savePublicCache = function (url, res, cache) {
    var self = this;
    // var CC = CM.CacheControl(res);
    var CC = CMIOREDIS.CacheControlIORedis(res);
    LOG2(self.LogHeader, "[meap_rm_robot_context][RBTContext][savePublicCache]:", url, CC);
    // this.Context.CacheMan.saveCache(url, "public", CC, cache, null); 原方法
    this.Context.CacheManIORedis.saveCache(url, "public", CC, cache, null);
}
RBTContext.prototype.savePublicEXPCache = function (url, res, cache, expdate) {
    ...
    LOG2(self.LogHeader, "[meap_rm_robot_context][RBTContext][savePublicIORedisEXPCache]:", url, CC);
    this.Context.CacheManIORedis.saveCache(url, "public", CC, cache, expdate);
}
...
RBTContext.prototype.savePrivateCache = function (url, res, cache) {
    ...
    this.Context.CacheManIORedis.saveCache(url, this.Auth.sid, CC, cache, null);
}
RBTContext.prototype.savePrivateEXPCache = function (url, res, cache, expdate) {
    ...
    this.Context.CacheManIORedis.saveCache(url, this.Auth.sid, CC, cache, expdate);
}
RBTContext.prototype.checkPrivateCache = function (url, cb) {
    ...
    this.getIoRedisCacheCC(url, cb, this.Auth.sid);
}
RBTContext.prototype.getPrivateCache = function (url, cb) {
    ...
    self.getIoRedisCache(url, this.Auth.sid, cb);
}
...
RBTContext.prototype.savePrivateCacheByUser = function (url, res, cache) {
    ...
    var self = this;
    var CC = CMIOREDIS.CacheControl(res);
    LOG3(self.LogHeader, "[meap_rm_robot_context][RBTContext][savePrivateCacheByUser]:CC is ", CC, "--", this.Get("MASMark"));
    this.Context.CacheManIORedis.saveCache(url, this.Get("MASMark"), CC, cache, null);
}
RBTContext.prototype.savePrivateByUserEXPCache = function (url, res, cache, expdate) {
    ...
    this.Context.CacheManIORedis.saveCache(url, this.Get("MASMark"), CC, cache, expdate);
}
RBTContext.prototype.checkPrivateByUserCache = function (url, cb) {
    ...
    this.getIoRedisCacheCC(url, cb, this.Get("MASMark"));
}
RBTContext.prototype.getPrivateByUserCache = function (url, cb) {
    ...
    self.getIoRedisCache(url, this.Get("MASMark"), cb);
}

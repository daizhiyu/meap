...
RedisManager.prototype.Init = async function (self) {
    self.redisClients={};
    try {
        var serviceObj=  JSON.parse(fs.readFileSync(self.workpath+"/service.json"))
        if(serviceObj['meap'] && serviceObj['meap']['redisClients']) {
            var redisClients = serviceObj['meap']['redisClients'];
            for (var m in redisClients) {
                self.redisClients[m] = new ioRedis(redisClients[m])
                if (redisClients[m].hasOwnProperty('sentinels') && redisClients[m].db){
                    self.redisClients[m].select(redisClients[m].db);
                }
                _INFO("[meap_rm_robot_manager_context] 构造ioredis 新对象", "redisClients." + m, redisClients[m]);
            }
        }
    } catch (e) {
        _INFO(e)
